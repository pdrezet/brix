...
#define IDCLGB_FOLDERTREE		0x3741
#define IDCLGB_TITLE			0x3742
#define IDCLGB_STATUSTEXT		0x3743

#define IDCLGB_NEW_EDIT_PATH	0x3744

// Class CDlgWnd
BEGIN_MESSAGE_MAP(CPathDialogSub, CWnd)
        ON_BN_CLICKED(IDOK, OnOK)
		ON_EN_CHANGE(IDCLGB_NEW_EDIT_PATH, OnChangeEditPath)
END_MESSAGE_MAP()

void CPathDialogSub::OnOK()
{
	::GetWindowText(::GetDlgItem(m_hWnd, IDCLGB_NEW_EDIT_PATH),
	...
	{
		::SetFocus(::GetDlgItem(m_hWnd, IDCLGB_NEW_EDIT_PATH));
	}
}
	...
	case BFFM_INITIALIZED: 
		{
			...
			::ShowWindow(::GetDlgItem(hwnd, IDCLGB_STATUSTEXT), SW_HIDE);
			::GetWindowRect(::GetDlgItem(hwnd, IDCLGB_FOLDERTREE), &rc);
			rc.bottom = rc.top - 4;
			rc.top = rc.bottom - 23;
			...
			hEdit = ::CreateWindowEx(WS_EX_CLIENTEDGE, _T("EDIT"), _T(""),
				...
				hwnd, NULL, NULL, NULL);
			::SetWindowLong(hEdit, GWL_ID, IDCLGB_NEW_EDIT_PATH);
	...
	case BFFM_SELCHANGED:
		{
			...
			::SetWindowText(::GetDlgItem(hwnd, IDCLGB_NEW_EDIT_PATH), szSelection);
			break;
		}
	default:
		break;
...
//return 0: ok
//return 1: error
/* Deprecated? if not, we need to determine the maximum size of lpRoot to avoid buffer overruns.
