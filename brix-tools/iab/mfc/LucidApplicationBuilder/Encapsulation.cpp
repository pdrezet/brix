...
void Encapsulation::EncapsulateBlock(INXObjList* _encapsulated, HTREEITEM _hItem)
{
	...
	if (iconNum > 0) {
		...
		AfxMessageBox("WARNING: Too many ports. Cannot encapsulate this block.");
	}
	...
	while (!nameOK) {
		...
		if (dialog->DoModal()==IDOK) {
			if (dialog->m_BlockName == "") {
				AfxMessageBox("WARNING: Enter a Block Name.");
				errorFlag = TRUE;
			}
			else if (pFrame->m_wndProjectBar.m_cProjTree.NameExist(dialog->m_BlockName, hItem)) {
				AfxMessageBox("WARNING: Block Name already exists. Choose a different name.");
				errorFlag = TRUE;
			}
			else if (dialog->m_BlockName.Find(" ") != -1) {
				AfxMessageBox("WARNING: Block Name is not allowed to contain spaces.");
				errorFlag = TRUE;
			}
			else {//filename OK
