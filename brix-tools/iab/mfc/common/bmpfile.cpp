									...
									
{
	...
	if (dataBuf==NULL)
		return NULL;
	UINT widthPix=bmiHeader.biWidth;
	UINT height=bmiHeader.biHeight;
...
//UINT *width, UINT *height,BYTE* planes,BYTE *bp
{
	...
	if (error!=0) {             
		...
		if (rc==-1) {AfxMessageBox("Read Error!"); fclose(fp); return NULL;}
		...
		if (b.bp==24) {
			...
			AfxMessageBox("Error - Bitmap not 24 or 32 bit");
		}
		// alloc our buffer
