...
//	void Draw(CDC * pDC, bool _onlyDrawAnim, int _toggleAnim);
	...
	int GetOutLineID(INXObjList* flattened, ConData* otherBlockIcon, ConData* flatBlob, int outNum , INXTREEITEM hItem);
	...
	int OnLine(INXPoint point, INXPOSITION* Icon, bool* nodeSel, int* portType);
	...
	void SetCondataLineID(INXObjList* flattened);
	...
	void RenameSubsystem(INXString csOldInstName, INXString csNewInstName, INXTREEITEM hUserdefItem);
