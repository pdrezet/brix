...
//#include "LabLgbBaseApp.h" //@todo
#include "LucidTypedefs.h" //@todo
...
void ProjectMetaData::getVersionInts(void)////
{
	...
	assert(dotPos > 0);
	INXString temp1(m_csLucidVersion.Left(dotPos));
	INXString temp2(m_csLucidVersion.Right(m_csLucidVersion.GetLength() - dotPos -1 ));
	m_iProjReadMajorRev = atoi( (char*)temp1);
	m_iProjReadMinorRev = atoi( (char*)temp2);
...
#endif
LucidErrEnum ProjectMetaData::showOpenProjectDialog(wxWindow *parent)//// _FIND_ME_
{
#ifdef _FIND_ME_
...
//		m_csProjectDir = dlg.m_ProjectDir + "\\" + dlg.m_ProjectName;
	  ...
	  wxString wildcards =
                    ...
                    wxString::Format
                    (
                        _T("All files (%s)|%s| files(*.lpj)|*.lpj"),
                        wxFileSelectorDefaultWildcardStr,
                        wxFileSelectorDefaultWildcardStr
                    );
#endif
	  
	wxFileDialog dialog(parent, _T("Testing open multiple file dialog"),
                        DEFAULTPROJECTROOTDIR, wxEmptyString, wildcards,
...
//		m_csProjectDir = dlg.m_ProjectDir + "\\" + dlg.m_ProjectName;
		...
		const char* data_char = (char*) m_csProjectDir.c_str();
		//AfxGetApp()->WriteProfileString(_T("Project"),_T("ProjectDir"),m_csProjectDir);
...
LucidErrEnum ProjectMetaData::getGuiFiles(vector<GuiFileMonty_t> &guiFileVec)
{
	...
	while(it != m_vGuiFiles.end()){
		...
		//err = getScreenTagDescription(guiFM.tag, dummyStr); @todo
		assert(  err == kErr_NoErr );
		guiFM.tagDescr = dummyStr;
...
void ProjectMetaData::updateTransferrables(const bool &bAppUpload)//// _FIND_ME
{
	...
	err = 0;//@todo ((CLabLgbBaseApp *)AfxGetApp())->getActiveImages( this, vImageFiles );
	assert(kErr_NoErr== err);
	...
	err = 0;//@todo((CLabLgbBaseApp *)AfxGetApp())->getActiveFonts( this, vFontNames );
	assert(kErr_NoErr== err);
...
void ProjectMetaData::setDirtySODLFlag()////
{
	if (!isDirtySODL) {
		...
		if (pFile == NULL) {
			// not expecting this
			wxMessageBox(wxT("Failed to create file for project in: ") + updateSODLFileName);
		} else {
			//success
			EhsFclose(pFile);
		}
...
int ProjectMetaData::readBuildNumFromFile(void)////
{
	...
	int num;
	ifstream read;
	INXString tempPchar(csProjectDir + (wxChar*)("\\build.num"));
	read.open((const char*)tempPchar.c_str(),ios_base::in);
	if ( read.is_open() ) {
