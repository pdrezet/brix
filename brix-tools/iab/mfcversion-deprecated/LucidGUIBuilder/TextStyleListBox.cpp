...
TextStyleListBox::TextStyleListBox(void)
{
}
...
void TextStyleListBox::SetTextStylesVec(vector<InlineText> vTextStylesVec)
{
	m_vTextStylesVec = vTextStylesVec;
}

// Method that returns the font name for a text style in the vector
CString TextStyleListBox::GetFont(CString csStyle)
{
	CString csFont = "";

	for (size_t i=0; i<m_vTextStylesVec.size(); i++) {
		if (m_vTextStylesVec[i].GetClass() == csStyle) {
			csFont = m_vTextStylesVec[i].GetFont();
		}
	}
...
void TextStyleListBox::GetColors(CString csStyle, LgbColorsDef &cColors)
{
	for (size_t i=0; i<m_vTextStylesVec.size(); i++) {
		if (m_vTextStylesVec[i].GetClass() == csStyle) {
			m_vTextStylesVec[i].GetColors(cColors);
		}
	}
}

/*
void TextStyleListBox::OnPaint()
{
	...
	// initialize
	pSrcBmp->CreateCompatibleBitmap(&dc, rArea.Width(), rArea.Height());
	pSrcMemDC->SelectObject(pSrcBmp);

	// RGB + Alpha
	int totalBytes = rArea.Width() * rArea.Height() * 4;
	...
	//Draw the Text
	//dc.TextOut(rArea.left, rArea.top, csStyle);
	...
	//if (0) {
		if (pFont) {
			...
			xBgColour = EhsGraphicsColour_argb(255, 255, 255, 0);
			EhsWidgetTable_init(&EhsWidgetTable);
			pWidget = EhsWidgetTextbox_init(rArea.left, rArea.top, 0, rArea.Width(), rArea.Height(), xFgColour, xBgColour, pFont);
			EhsWidget_create(pWidget);
			EhsGraphicsColourClass xPalette[2];
			xPalette[0] = EHS_WIDGET_TEXTBOX(pWidget).xBgColour;
			xPalette[1] = EHS_WIDGET_TEXTBOX(pWidget).xFgColour;
			...
			for (y = 0; y < rArea.Height(); y++) {
				int nPixIdx = y * rArea.Width() * 4;
				int nBmpIdx = y * ((pSurface->nWid+7)/8);
				for (x = 0; x < rArea.Width(); x++) {
					int nBit = 1 << (7u - (x % 8u));
					int nX = x*4;
					if ((nBit & EhsTgtViewportSurface_bitmap(pSurface)[nBmpIdx + x/8]) == nBit) {
						tmp[nPixIdx+nX] = (unsigned char) cColors.m_iFgBlue;
