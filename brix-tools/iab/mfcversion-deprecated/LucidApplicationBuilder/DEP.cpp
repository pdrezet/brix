...
bool areBothIconsXPorts(ConData *pFirstIcon, ConData *pSecondIcon)
{
	...
	if (pFirstIcon && pSecondIcon) {
		if (pFirstIcon->m_csIconType.Find("XINPUT") != -1 || pFirstIcon->m_csIconType.Find("XOUTPUT") != -1 || pFirstIcon->m_csIconType == "XSTART" || pFirstIcon->m_csIconType == "XFINISH") {
			bFirstIsXport = true;
		}
		if (pSecondIcon->m_csIconType.Find("XINPUT") != -1 || pSecondIcon->m_csIconType.Find("XOUTPUT") != -1 || pSecondIcon->m_csIconType == "XSTART" || pSecondIcon->m_csIconType == "XFINISH") {
			bSecondIsXport = true;
		}
...
int DEP::AddLine(INXPOSITION selectedIcon, int selectedPort, int selectedPortType, INXPOSITION selectedIcon2,int selectedPort2, int selectedPortType2) {
	...
	if (selectedPortType == STARTPORT) {
		...
		if (iconIn->startport[selectedPort]->initialise) {
			AfxMessageBox("Illegal connection. Start port is already connected to initialiser.");
	...
	{
		AfxMessageBox("This is an illegal connection.");
		depFSM.drawingline = 0;
		return 0;
	}

	if (areBothIconsXPorts(iconIn, iconOut)) {
		AfxMessageBox("Illegal connection. Start and end points are Xports.");
		depFSM.drawingline = 0;
		return 0;
	}
...
int DEP::AddPort(ConData* blob, int iDataType, int iPortType, INXString portLabel) {
	...
	{
		AfxMessageBox("No more ports can be added as you have the maximum.");
		return 0;
	}
...
int DEP::AddTag(INXPOSITION iconPos, int portNum, int portType) {
	...
	if (portType == INPUTPORT) {
		if (blob->inputport[portNum]->connected) {
			AfxMessageBox("Input port is already connected.");
			ret = 0;
		}
		...
		if (blob->startport[portNum]->connected) {
			AfxMessageBox("Start port is already connected.");
			ret = 0;
		}
		else if (dialog.DoModal() == IDOK) {
...
INXString DEP::DeletePort(ConData* blob, int portNum, int portType) {
	...
	default:
		AfxMessageBox("Data type doesn't exist"); break;
	}
	return portLabel;
}
		...
		while(flatPos) {
			...
			// make sure its not an xport
				...
				if (blob->m_csIconType == flatBlob->m_csIconType) {
					...
					if (!errorFlag) {
						errorFlag = TRUE;
						AfxMessageBox("The mapping has failed. Debug may not work as expected.");
					}
					//AfxMessageBox("The icons have same id but are not of same type");
				}
...
void DEP::ShowLine(INXPOSITION iconPos, int portNum, int portType) {
	...
	if (portType == INPUTPORT) {
		if (blob->inputport[portNum]->line.exist) {
			...
			AfxMessageBox("This port is not connected.");
		}
	}
	else if (portType == STARTPORT) {
...
ConData* DEP::SubsetEncapsulate(INXRect encapsulate, int lib) {
	...
	if (subBlock.HasXPorts(encapsulate, depList)) {
		AfxMessageBox("Currently, it is not possible to encapsulate Xports. Please try again.");
	}
	else {
			...
			if (!pToIcon->inputport[iSelectedPort2]->connected) {
				if (pFromIcon->inputport[iSelectedPort]->datatype == pToIcon->inputport[iSelectedPort2]->datatype) {
					...
					if (areBothIconsXPorts(otherBlob, pToIcon)) {
						AfxMessageBox("Cannot drag connections, because start and end points are Xports.");
					} else {
					...
					AfxMessageBox("Cannot drag connections, because the datatypes do not match.");
				}
			}
			else {
				AfxMessageBox("Cannot drag connections, because the target port is already connected.");
			}
			break;
		}
	case 1 :	// output port
	...
	case 2 :	// start port
		{
			if (!pToIcon->startport[iSelectedPort2]->connected && !pToIcon->startport[iSelectedPort2]->initialise) {
				...
				AfxMessageBox("Cannot drag connections, because the target port is already connected or initialised.");
			}
			break;
		}
	case 3 :	// finish port
