...
void ConData::init(INXString csIconType, INXString csBlockName, INXPoint point, int iShow) {
	...
	if(m_csIconType == "")
	{
		AfxMessageBox("Icon type is empty");
	}
...
void ConData::initBmp(INXPoint _point) 
{
	...
	bitmappath = workDir + CDFDIR + m_csIconType + ".bmp";
	if (!fo.CheckPath(bitmappath)) {
		bitmappath = workDir + BMPDIR + m_csIconType + ".bmp";
		if (!fo.CheckPath(bitmappath)) {
			AfxMessageBox("Unable to open " + bitmappath);
			return;
		}
	}
...
int ConData::AddLine(int inPortNum, int inPortType, ConData* othericon,int otherPortNum, int otherPortType){ 	
	...
	if (inPortType == INPUTPORT ) {
		...
		if (inputport[inPortNum]->connected) {
			AfxMessageBox("Cannot connect more than one output to an input");
			return 0;
		}
...
int  ConData::CheckDatatypes(int portIn, int portInType, ConData*iconOut,int portOut,int portOutType) {
	...
	if  (GetPortDataType(portIn, portInType)!=iconOut->GetPortDataType(portOut,portOutType)) {
		AfxMessageBox("Cannot connect different data types");
		return -1;	
	}
	return 1;
}
 ...
 */
void ConData::DrawTitle(CDC* theDC) {
...
//	*file >> temp; //read the BEGIN_CONDATA MARKER
	...
	*file >> showdescript;
	...
	if (description=="_") description="";  //put back to normal
	if (optionstring=="_") optionstring="";
	if (longDesc=="_") longDesc="";
	if (hierarchyName=="_") hierarchyName = "";
...
{
	if (description=="") description="_";	//make sure no empty bits (spaces)
	if (optionstring=="") optionstring="_";
	if (longDesc=="") longDesc="_";
	if (m_csBlockName=="") m_csBlockName = "_";
	if (hierarchyName=="") hierarchyName="_";
	*file << endl<<(LPCTSTR)(m_csIconType)<<endl;
	*file << (LPCTSTR)(m_csBlockName) << endl;
	...
	*file << (LPCTSTR)(description)<<endl;
	*file << (LPCTSTR)optionstring<<endl;
	*file << (LPCTSTR)longDesc<<endl;
	*file << (LPCTSTR)className<<endl;
	*file << (LPCTSTR)hierarchyName<<endl;
	...
	while (csPortType != "") {
		...
		for (int i=0; i<vPortKeysVec.size(); i++) {
			if (vPortKeysVec[i].Find(KEYDESC) != -1) {
				...
				//AfxMessageBox(descKey + "=" + portDesc);
			}
...
void ConData::ReadIDFFile(INXString csIconType, INXPoint point) {
	...
	if (pos == -1) {
		...
		if (!fo.CheckPath(filepath)) {
			...
			if (!fo.CheckPath(filepath)) {
				AfxMessageBox("No IDF defined for Component: " + csIconType);
				return;
			}
		}
...
//			filepath = workDir + USERDEFDIR + csIconType + ".idf.ini";
//			if (!fo.CheckPath(filepath)) {
				AfxMessageBox("No CDF defined for Component: " + csIconType);
				return;
//			}
		}
