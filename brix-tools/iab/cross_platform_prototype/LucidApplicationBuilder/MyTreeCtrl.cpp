...
//#include "../common/General_Utilities/GlobalFuncs_3.h" //@todo
//#include "userDefinedMessages.h" //@todo
...
//	txtItem = InsertItem(_T("TXT"), 3, 3, projItem); //dk - Data Items removed from IAB
	...
	for (unsigned int  i=0; i<guiFileVec.size(); i++) {
	...
	for (unsigned int i=0; i<pngFileVec.size(); i++) {
...
*/
	...
	for (unsigned int i=0; i<vNlsFileVec.size(); i++) {
	...
	for (unsigned int i=0; i<xResourceFileVec.size(); i++) {
...
void CMyTreeCtrl::CloseProject(/*HTREEITEM hItem,*/ Project* activeProject) {////
	...
	activeProject->SaveProject();
///	((CDrawProgApp*)AfxGetApp())->CloseProject(activeProject); //@todo
#ifdef TRY_EXCLUDE
